    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 border-b border-border">
      {items.map((item, i) => (
        <div
          ...
          style={{
            animation: `lift-in 460ms cubic-bezier(0.22,1,0.36,1) both ${i * 50}ms`,
          }}
        >
          <div className="eyebrow">{item.label}</div>
          <div
            className={cn(
              "mt-2.5 font-semibold tracking-[-0.02em] tnum text-[2rem] leading-none sm:text-[2.5rem]",
              item.accent ? "text-accent-brand" : "text-foreground",
            )}
          >
            {item.value}
            {item.suffix && (
              <span className="ml-1 text-base text-muted-foreground font-medium">
                {item.suffix}
              </span>
            )}
          </div>
  ...
  return (
    <div className="min-w-0 space-y-12">
      <div>
        ...
        <div className="mt-3 flex h-1.5 overflow-hidden rounded-sm bg-muted/40">
          {total === 0 ? (
        ...
        <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-x-5 gap-y-3">
          {buckets.map((b) => (
            <div key={b.key} className="flex items-center gap-3 min-w-0">
              <span
                className="inline-block h-3 w-0.75 rounded-sm shrink-0"
          ...
          {stats.recentRuns.slice(0, 10).map((run, idx) => (
            <li key={run.id}>
              <Link
                ...
                style={{
                  animation: `lift-in 380ms cubic-bezier(0.22,1,0.36,1) both ${24 * idx}ms`,
                }}
              >
                <div className="flex-1 min-w-0 space-y-1">
                  <div
                    className="truncate text-sm font-medium text-foreground group-hover:text-accent-brand transition-colors"
                    title={run.jobName}
                  >
                    {run.jobName}
                  </div>
    ...
    <>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 border-b border-border">
        {Array.from({ length: 5 }).map((_, i) => (
          <div
            key={i}
            className={cn("px-5 py-5", i === 0 && "pl-6", i === 4 && "pr-6")}
          >
