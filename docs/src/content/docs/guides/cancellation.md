...
Cancellation transitions the run directly to `Canceled`. The running attempt receives a signal through its `CancellationToken` and stops at the next opportunity. If a running job never checks its token, it keeps executing until it finishes on its own or the process shuts down, even though the run is already marked `Canceled`.

## Cascade cancellation

When you cancel a run, all of its child runs (triggered via `IJobClient` from within the job) are also canceled. This walks the entire trace tree, so grandchildren and deeper descendants are canceled too.

```csharp
app.AddJob("Parent", async (IJobClient client, CancellationToken ct) =>
{
    // If Parent is canceled, this child is also canceled.
...
A timeout cancels the running attempt's `CancellationToken` and records the attempt as failed (with a `TimeoutException`), not as canceled. Normal retry policy applies, so a timeout that has retries remaining produces a fresh attempt.

## Shutdown

When the host shuts down, in-flight runs are canceled and have up to `ShutdownTimeout` (default 15 seconds) to finish. Anything still running past the timeout is recorded as a failure and follows normal retry policy.
