...
public abstract class BatchConformanceTests : StoreConformanceBase
{
    ...
    public async Task IncrementBatchCounter_Completed_IncrementsCompleted()
    {
        ...
        Assert.Equal(5, counters.Total);
        Assert.Equal(1, counters.Completed);
    ...
    public async Task IncrementBatchCounter_Failed_IncrementsBoth()
    {
        ...
        var counters = await Store.IncrementBatchCounterAsync(run.Id, true);
    ...
    public async Task IncrementBatchCounter_Returns_PostIncrementValues()
    {
        ...
        var c1 = await Store.IncrementBatchCounterAsync(run.Id, false);
        Assert.Equal(1, c1.Completed);

        var c2 = await Store.IncrementBatchCounterAsync(run.Id, false);
        Assert.Equal(2, c2.Completed);

        var c3 = await Store.IncrementBatchCounterAsync(run.Id, true);
        Assert.Equal(2, c3.Completed);
        Assert.Equal(1, c3.Failed);
    }
    ...
    public async Task IncrementBatchCounter_Concurrent_NoLostIncrements()
    {
        ...
        for (var trial = 0; trial < 20; trial++)
        {
            ...
            var tasks = Enumerable.Range(0, 100)
                .Select(_ => Store.IncrementBatchCounterAsync(run.Id, false))
                .ToArray();
    ...
    public async Task IncrementBatchCounter_CrossesThreshold_ExactlyOnce()
    {
        for (var trial = 0; trial < 20; trial++)
        {
            ...
            var crossedCount = results.Count(c => c.Completed + c.Failed == c.Total);
    ...
    public async Task IncrementBatchCounter_AlreadyComplete_NoOp()
    {
        ...
        Assert.Equal(5, counters.Total);
        Assert.Equal(5, counters.Completed);
        Assert.Equal(0, counters.Failed);
    }

    [Fact]
    public async Task IncrementBatchCounter_NonExistentRun_Throws()
    {
        var fakeId = Guid.CreateVersion7().ToString("N");

        await Assert.ThrowsAsync<InvalidOperationException>(() => Store.IncrementBatchCounterAsync(fakeId, false));
    }

    [Fact]
    public async Task IncrementBatchCounter_NormalRun_Throws()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() => Store.IncrementBatchCounterAsync(run.Id, false));
    }
}