...
const relativeFormat = new Intl.RelativeTimeFormat(undefined, { numeric: 'always', style: 'narrow' });
...
export function parseTimeSpan(ts?: string | null): number | null {
  ...
  const whole = ((Number(d ?? 0) * 86400) + (Number(h) * 3600) + (Number(m) * 60) + Number(s)) * 1000;
