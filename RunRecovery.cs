...
{
    /// <summary>
    /// Attempts to recover an inactive run by atomically transitioning it from Running to Failed/DeadLetter.
    ...
    internal async Task<bool> TryRecoverRunAsync(JobRun run, string reason, CancellationToken ct = default)
    {
        ...
        // recovery paths (startup, inactive-run, inactive-node) race on the same run.
        ...
        if (canRetry)
        {
            ...
            var retryRun = JobRun.CreateRetry(run, retryDelay, timeProvider.GetUtcNow());
        ...
        // Append status event (includes retryRunId so WaitAsync can read it from the store)
