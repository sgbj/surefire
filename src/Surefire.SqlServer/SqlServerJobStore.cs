...
internal sealed class SqlServerJobStore(string connectionString, TimeProvider timeProvider) : IJobStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        cmd.CommandText = """
                          ...
                          """;
        cmd.Parameters.Add(CreateParameter("@id", batchId));
        cmd.Parameters.Add(CreateParameter("@status", (int)terminalStatus));
    ...
    private static JobRun ReadRun(SqlDataReader reader) => new JobRun
    {
    ...
    private static JobBatch ReadBatch(SqlDataReader reader) => new JobBatch
    {
        Id = reader.GetString(reader.GetOrdinal("id")),
