...
public sealed class HandlerMetadataTests
{
    ...
    public void AsyncEnumerableElementType_Set_ForTaskOfIAsyncEnumerableReturn()
    {
        ...
        Assert.Equal(typeof(int), meta.AsyncEnumerableElementType);
        Assert.NotNull(meta.Materializer);
    }

    [Fact]
    public void AsyncEnumerableElementType_Set_ForValueTaskOfIAsyncEnumerableReturn()
    {
        Delegate handler = () => ValueTask.FromResult(AsyncEnumerable.Empty<int>());
        var meta = HandlerMetadata.Build(handler);
