...
export function TriggerDialog({
  ...
  const requiredFields = useMemo(
    () => new Set(argumentsSchema?.required ?? []),
    [argumentsSchema],
  );
  ...
  const buildArgs = (): unknown | undefined => {
    ...
    for (const [name, prop] of Object.entries(properties)) {
      ...
      const resolvedType = argumentsSchema
        ? resolveType(resolveSchemaProperty(prop, argumentsSchema))
        : resolveType(prop);
  ...
  const handleRun = () => {
    ...
    if (notBefore) {
      const notBeforeUtc = toUtcIsoString(notBefore);
      if (!notBeforeUtc) {
        toast.error("Invalid Not before date");
        return;
      }
      opts.notBefore = notBeforeUtc;
    }
    if (notAfter) {
      const notAfterUtc = toUtcIsoString(notAfter);
      if (!notAfterUtc) {
        toast.error("Invalid Not after date");
        return;
      }
    ...
    if (opts.notBefore && opts.notAfter && opts.notAfter <= opts.notBefore) {
      toast.error("Not after must be later than Not before");
      return;
    }
      ...
      <DialogContent className="max-w-lg max-h-[85vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription className="sr-only">
            Configure arguments and options for this job run.
          </DialogDescription>
        </DialogHeader>
          ...
          <Collapsible>
            ...
            <CollapsibleContent className="pt-3">
              <div className="grid grid-cols-2 gap-3">
                <Field>
                  <FieldLabel htmlFor="trigger-not-before">
                    Not before
                  </FieldLabel>
                  <Input
                ...
                <Field>
                  <FieldLabel htmlFor="trigger-dedup">
                    Deduplication ID
                  </FieldLabel>
                  <Input
                    id="trigger-dedup"
...
function resolveJsonPointer(
  ...
  for (const segment of segments) {
    ...
    throw new Error(`Invalid schema reference: #${pointer}`);
  }
...
function resolveSchemaProperty(
  ...
  if (prop.$ref && prop.$ref.startsWith("#/")) {
    const pointer = prop.$ref.slice(1);
    if (seenRefs.has(pointer)) return prop;

    seenRefs.add(pointer);
  ...
  if ((prop.oneOf || prop.anyOf) && !prop.type) {
    const variants = prop.oneOf ?? prop.anyOf ?? [];
    const firstNonNull = variants.find((v) => {
      const t = v.type;
      if (!t) return false;
      if (Array.isArray(t)) return t.some((x) => x !== "null");
      return t !== "null";
    });

    if (firstNonNull) {
      return resolveSchemaProperty(firstNonNull, root, seenRefs);
    }
  }
      ...
      <Field>
        ...
        <select
          ...
          onChange={(e) => {
            ...
            const original = resolvedSchema.enum!.find(
              (v) => String(v) === selected,
            );
