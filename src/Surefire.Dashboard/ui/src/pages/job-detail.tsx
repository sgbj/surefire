  ...
  const metadataItems: MetadataItem[] = [
    ...(job.description
      ? [{
        ...
        label: "Description",
        fullWidth: true,
        children: <span className="text-sm text-foreground/85">{job.description}</span>,
      }]
      : []),
    {
      key: "schedule",
    ...
    ...(job.nextRunAt
      ? [{key: "nextRun", label: "Next run", align: "mono" as const, children: formatDate(job.nextRunAt)}]
      : []),
    ...(stats && stats.totalRuns > 0
      ? [
        {key: "totalRuns", label: "Total runs", align: "mono" as const, children: stats.totalRuns},
        {key: "successRate", label: "Success rate", align: "mono" as const, children: `${stats.successRate.toFixed(1)}%`},
      ]
      : []),
    ...(job.maxConcurrency != null
      ? [{key: "maxConcurrency", label: "Max concurrency", align: "mono" as const, children: job.maxConcurrency}]
      : []),
    ...(job.timeout
      ? [{key: "timeout", label: "Timeout", align: "mono" as const, children: formatTimeSpan(job.timeout)}]
      : []),
    ...(job.retryPolicy.maxAttempts > 1
      ...
      : []),
    ...(stats?.avgDuration
      ? [{key: "avgDuration", label: "Avg duration", align: "mono" as const, children: formatTimeSpan(stats.avgDuration)}]
      : []),
    ...(stats?.lastRunAt
      ? [{key: "lastRun", label: "Last run", align: "mono" as const, children: formatDate(stats.lastRunAt)}]
      : []),
    ...(job.tags.length > 0
      ? [{
        key: "tags",
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="gap-0">
        <div className="px-6 pt-4">
          <TabsList className="bg-card/40 border border-border rounded-md p-0.5">
            ...
            {job.sourceCode && <TabsTrigger value="source">Source</TabsTrigger>}
          </TabsList>
        </div>
        ...
        {job.sourceCode && (
          <TabsContent value="source" className="mt-0">
            {activeTab === "source" && (
              <JobSourcePanel code={job.sourceCode}/>
            )}
          </TabsContent>
        )}
      </Tabs>
...
function JobSourcePanel({code}: { code: string }) {
  return (
    <div className="relative mt-5 border-y border-border bg-card/40">
...
function HighlightedSource({code}: { code: string }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .then(([hljsModule, csharpModule]) => {
        ...
        if (!hljs.getLanguage("csharp")) hljs.registerLanguage("csharp", csharpModule.default);
  ...
  if (html) {
    return (
      <pre className="job-source max-h-[36rem] overflow-auto whitespace-pre bg-transparent px-6 py-4 text-xs leading-[1.55] font-mono">
        <code className="hljs language-csharp !bg-transparent" dangerouslySetInnerHTML={{__html: html}}/>
      </pre>
    );
  }

  return (
    <pre className="max-h-[36rem] overflow-auto whitespace-pre px-6 py-4 text-xs leading-[1.55] font-mono text-foreground/85">
      <code>{code}</code>
    </pre>
  );
}
